...
*   **[FIX_ELIMINACION_CIERRES_MODAL_PORTAL_LAYOUT_14_JULIO_2026.md](fixes/FIX_ELIMINACION_CIERRES_MODAL_PORTAL_LAYOUT_14_JULIO_2026.md)**
    *   *Descripción:* Implementación del endpoint de eliminación de cierres mensuales (`DELETE /api/counters/monthly/{id}`) con multi-tenancy y cascade. Reescritura del componente `Modal` con `ReactDOM.createPortal` para corregir el descentrado causado por CSS Stacking Context (`backdrop-filter`). Corrección del layout full-screen del módulo de Contadores. Suite de 22 pruebas unitarias (100% PASS).
...
*   **[ARQUITECTURA_DETALLADA_Y_PATRONES.md](arquitectura/ARQUITECTURA_DETALLADA_Y_PATRONES.md):** Detalle de capas lógicas (Clean Architecture), patrón repositorio, concurrencia de scraping de impresoras y arquitectura frontend (React + Zustand).
...
*   **[INFRAESTRUCTURA_Y_STACK_TECNOLOGICO.md](arquitectura/INFRAESTRUCTURA_Y_STACK_TECNOLOGICO.md):** Especificación completa de la infraestructura de producción, contenedores Docker, red virtual, volúmenes de persistencia y stack de seguridad.
*   **[ADR_DECISIONES_DISENO.md](arquitectura/ADR_DECISIONES_DISENO.md):** Registro formal de Decisiones de Arquitectura (ADR) que detalla justificaciones técnicas para tecnologías y decisiones críticas.
*   **[DIAGRAMA_C4_MODEL.md](arquitectura/DIAGRAMA_C4_MODEL.md):** Diagrama y modelado visual C4 de Contexto, Contenedores y Componentes del sistema.
...
*   **[PLAN_RECUPERACION_ANTE_DESASTRES.md](deployment/PLAN_RECUPERACION_ANTE_DESASTRES.md):** Plan ante desastres (DRP) detallando copias de seguridad de Postgres/SQLite y pasos de restauración.
*   **[MONITOREO_Y_ALERTAS.md](deployment/MONITOREO_Y_ALERTAS.md):** Guía operativa de logs en Docker/Nginx/SQLite y solución rápida de excepciones y rate limit.
...
*   **[GUIA_CONFIGURACION_ENTORNO.md](desarrollo/GUIA_CONFIGURACION_ENTORNO.md):** Requisitos de software, .env, formateadores sugeridos (Prettier/Black) y extensiones VS Code.
*   **[HISTORIAS_USUARIO_Y_CRITERIOS_ACEPTACION.md](guias/HISTORIAS_USUARIO_Y_CRITERIOS_ACEPTACION.md):** Historias de usuario, casos de uso principales y criterios de QA para validación de tareas.
