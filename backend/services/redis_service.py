...
class RedisService:
    ...
    def __init__(self):
        ...
        try:
            ...
            if redis_url:
                # Use REDIS_URL if provided (production)
                self.client = redis.from_url(redis_url, decode_responses=True)
                logger.info(f"🔴 Conectando a Redis via URL: {redis_url.split('@')[-1] if '@' in redis_url else redis_url}")
            else:
                # Use individual config variables (development)
                ...
                logger.info(f"🔴 Conectando a Redis: {host}:{port}/{db}")
            
            # Test connection
            self.client.ping()
            self.enabled = True
            logger.info("✅ Redis conectado y operativo")
            logger.info(f"   Backend: Redis")
            logger.info(f"   Caché distribuido: Habilitado")
            
        except ImportError:
            logger.warning("⚠️  Módulo 'redis' no instalado")
            logger.warning("   Usando caché en memoria (solo desarrollo)")
            logger.warning("   Para instalar: pip install redis==5.0.1 hiredis==2.3.2")
            self.enabled = False
    ...
    def invalidate_pattern(self, pattern: str):
        ...
        if not self.enabled:
            ...
            logger.debug(f"Invalidados {len(keys_to_delete)} keys con patrón '{pattern}' (memoria)")
            return
        ...
        try:
            ...
            if keys:
                ...
                logger.debug(f"Invalidados {len(keys)} keys con patrón '{pattern}' (Redis)")
        except Exception as e:
    ...
    def get_stats(self) -> dict:
        ...
        if not self.enabled:
            ...
            return {
                ...
                "warning": "Usando caché en memoria (no recomendado para producción)"
            }
